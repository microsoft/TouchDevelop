...
var settingsOptionsJson: JsonObject;
...
function _initWorkspaces() : void
{
    ...
    settingsOptionsJson = JSON.parse(settingsOptions);
...
function setFields(bld: JsonBuilder, body: JsonObject, fields: string) : void
{
    for (let fld of fields.split("\n")) {
        if (body.hasOwnProperty(fld) && body[fld].kind() == bld[fld].kind()) {
            bld[fld] = body[fld];
        }
    }
}

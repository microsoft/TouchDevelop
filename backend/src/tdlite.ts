...
async function importOneAnythingAsync(js: JsonObject) : Promise<core.ApiRequest>
{
    ...
    if ( ! core.isGoodEntry(entry)) {
        ...
        if (kind == "script") {
            ...
            await tdliteArt.importArtAsync(apiRequest, js);
        }
        else if (kind == "review") {
            ...
            await tdliteArt.importScreenshotAsync(apiRequest, js);
        }
        else {
        ...
        if (existing == null) {
            ...
            if (! ptr.redirect) {
                if (! ptr.artid) {
                    ...
                    cont = "";
                    if (tdliteArt.thumbContainers.filter(elt => elt.name == cont).length == 0) {
                        cont = "pub";
                    }
    ...
    if (delEntry["kind"] == "review") {
        ...
        if (delok) {
            ...
            if (kind == "group") {
                ...
                await tdliteArt.artContainer.deleteBlobAsync(entryid);
                for (let thumbContainer of tdliteArt.thumbContainers) {
...
export async function deleteUserAsync(req8:core.ApiRequest)
{
    ...
    await deleteAllByUserAsync(tdliteArt.arts, req8.rootId, req8);
    await deleteAllByUserAsync(tdliteScripts.scripts, req8.rootId, req8);
    await deleteAllByUserAsync(pointers, req8.rootId, req8);
    await deleteAllByUserAsync(tdliteArt.screenshots, req8.rootId, req8);
